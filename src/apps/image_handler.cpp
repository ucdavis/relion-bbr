...
class image_handler_parameters
{
	...
	RFLOAT multiply_constant, divide_constant, add_constant, subtract_constant,
	Do_random_drop_for_raw_stack,
	Do_add_incoherent,Do_add_coherent,
	threshold_above, threshold_below, angpix, requested_angpix, real_angpix, force_header_angpix, lowpass, highpass, logfilter, bfactor, shift_x, shift_y, shift_z, replace_nan, randomize_at, optimise_bfactor_subtract;
	// PNG options
	RFLOAT minval, maxval, sigma_contrast;
	int color_scheme; // There is a global variable called colour_scheme in displayer.h!
	RFLOAT Drop_percentage;
	int Drop_start_frame_num,Drop_end_frame_num;
	...
	void read(int argc, char **argv)
	{
		...
		int Drop_section = parser.addSection("Drop electrons for raw image stacks, can only be used for K2 images");
		Do_random_drop_for_raw_stack = parser.checkOption("--do_random_drop", "Whether to random drop electron for K2 stacks.");
		Drop_percentage = textToFloat(parser.getOption("--drop_ratio", "Drop electron ratio, range from 0 to 1", "0."));
		Drop_start_frame_num = textToInteger(parser.getOption("--drop_first_frame", "first_frame_to_start_drop", "0"));
		Drop_end_frame_num = textToInteger(parser.getOption("--drop_last_frame", "last_frame_to_start_drop", "-1"));
		...
		Do_add_coherent = parser.checkOption("--add_coherent", "Coherently add mrcs");
		Do_add_incoherent = parser.checkOption("--add_incoherent", "Coherently add mrcs");
	...
	void run()
	{
		...
		Image<float> I_drop;
			...
			MultidimArray<RFLOAT> AMP,COUNT;
			AMP.resize(2000);
			AMP.initZeros();
			COUNT.resize(2000);
			COUNT.initZeros();
			MultidimArray<RFLOAT> REAL,IMAGEP;
			REAL.resize(2000);
			REAL.initZeros();
			IMAGEP.resize(2000);
			IMAGEP.initZeros();
		...
		if (Do_random_drop_for_raw_stack){
			if(Drop_end_frame_num<0 || Drop_end_frame_num>MD.numberOfObjects())Drop_end_frame_num=MD.numberOfObjects();
			if(Drop_start_frame_num<0)Drop_start_frame_num=0;
			if(Drop_start_frame_num>MD.numberOfObjects())Drop_start_frame_num=MD.numberOfObjects();
			if(fn_out.getExtension() == "tif" || fn_out.getExtension() == "tiff"){
				fn_tmp_tiff= fn_out + ".tmp";
				tiffff= TIFFOpen(fn_tmp_tiff.c_str(), "w");
				
			}
			
	//		std::cout<<"debug MD.numberOfObjects()"<<MD.numberOfObjects()<<std::endl;
		}
   		FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD)
		{
			...
			else if (Do_add_incoherent)
			{
				...
				Image<RFLOAT> TMPX;
				TMPX().resize(YSIZE(Iin()),XSIZE(Iin()));
				for(int II=0;II<ZSIZE(Iin());II++){
					TMPX().initZeros();
					FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY2D(TMPX())
					{
						DIRECT_A2D_ELEM(TMPX(),i,j) += DIRECT_A3D_ELEM(Iin(),II, i, j);;
					}
					transformer.FourierTransform(TMPX(), FT);
					FOR_ALL_ELEMENTS_IN_FFTW_TRANSFORM(FT)
					{
						int r2 = ip * ip + jp * jp;
						int RR=int(sqrt((RFLOAT)r2));
						if(RR<2000){
							DIRECT_A1D_ELEM(AMP, RR) += norm(DIRECT_A2D_ELEM(FT, i, j));
							DIRECT_A1D_ELEM(COUNT, RR)+=1.0;
						}
			...
			else if (Do_add_coherent)
			{
				...
				for(int II=0;II<ZSIZE(Iin());II++){
					...
					FOR_ALL_ELEMENTS_IN_FFTW_TRANSFORM(FT)
					{
						...
						int RR=int(sqrt((RFLOAT)r2));
						if(RR<2000){
							DIRECT_A1D_ELEM(REAL, RR) += DIRECT_A2D_ELEM(FT, i, j).real;
							DIRECT_A1D_ELEM(IMAGEP, RR) += DIRECT_A2D_ELEM(FT, i, j).imag;
							DIRECT_A1D_ELEM(COUNT, RR)+=1.0;
						}
					}
				}
			...
			else if (Do_random_drop_for_raw_stack){
				I_drop().resize(ydim, xdim);
				I_drop().initZeros();
				Iin.read(fn_img);
	//			std::cout<<"debug start, current_object="<<current_object<<", Drop_start_frame_num="<<Drop_start_frame_num<<", Drop_end_frame_num="<<Drop_end_frame_num<<std::endl;
	//			std::cout<<"debug start, xdim="<<xdim<<", ydim="<<ydim<<", fn_img="<<fn_img<<std::endl;

				if (current_object >= Drop_start_frame_num && current_object < Drop_end_frame_num)
				{
	//				std::cout<<"debug start 1, current_object="<<current_object<<std::endl;
					FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY2D(Iin())
					{
			//			std::cout<<"debug start2"<<std::endl;
						RFLOAT TMP_Element=DIRECT_A2D_ELEM(Iin(), i, j);
						if(TMP_Element<=0.2){continue;}
						if(TMP_Element>0.2){
							int TMP_X=ROUND(TMP_Element);
							for (int IterAtion=0; IterAtion<TMP_X;IterAtion++){
								if(Drop_percentage>rnd_unif(0.0,1.0)){
									TMP_Element=TMP_Element-1.0;
								}
							}
						}
						if(TMP_Element<0.0){TMP_Element=0.0;}
				//		std::cout<<"TMP_Element="<<TMP_Element<<std::endl;
						DIRECT_A2D_ELEM(I_drop(), i, j) = TMP_Element;
					}
				}
				else{
//					std::cout<<"debug start 2, current_object="<<current_object<<std::endl;
					FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY2D(Iin())
					{
						DIRECT_A2D_ELEM(I_drop(), i, j) =  DIRECT_A2D_ELEM(Iin(), i, j);
					}
					
				}
				if(fn_out.getExtension() == "tif" || fn_out.getExtension() == "tiff")
					TIFFConverter::write_tiff_one_page(tiffff, I_drop(), -1, COMPRESSION_LZW, 6, false);
				else
					I_drop.write(fn_out,-1,true,WRITE_APPEND);
	...
	//			std::cout<<"debug Go perImageOperations."<<std::endl;
		...
		if(fn_out.getExtension() == "tif" || fn_out.getExtension() == "tiff"){
			TIFFClose(tiffff);
			std::rename(fn_tmp_tiff.c_str(), fn_out.c_str());
		}
		if(Do_add_incoherent){
			FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY1D(AMP)
			{
				if(DIRECT_A1D_ELEM(COUNT,i )>0){
					DIRECT_A1D_ELEM(AMP,i)=sqrt(DIRECT_A1D_ELEM(AMP,i));
					DIRECT_A1D_ELEM(AMP,i) /=DIRECT_A1D_ELEM(COUNT,i);
				}
					
				std::cout<<i<<"\t"<<DIRECT_A1D_ELEM(AMP,i)<<std::endl;
			}
		}
		if(Do_add_coherent){
			FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY1D(REAL)
			{
				if(DIRECT_A1D_ELEM(COUNT,i )>0){
				
					DIRECT_A1D_ELEM(REAL,i) /=DIRECT_A1D_ELEM(COUNT,i);
					DIRECT_A1D_ELEM(IMAGEP,i) /=DIRECT_A1D_ELEM(COUNT,i);
				}
				RFLOAT TMPX_V=sqrt(DIRECT_A1D_ELEM(REAL,i)*DIRECT_A1D_ELEM(REAL,i)+DIRECT_A1D_ELEM(IMAGEP,i)*DIRECT_A1D_ELEM(IMAGEP,i));
				std::cout<<i<<"\t"<<TMPX_V<<std::endl;
			}
		}
