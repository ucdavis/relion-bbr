...
void Preprocessing::read(int argc, char **argv, int rank)
{
	...
	int addnoise_section = parser.addSection("Add noise during particle extraction");
	do_addnoise = parser.checkOption("--add_noise", "Add Gaussian noise to output particles");
	fourier_add_noise = parser.checkOption("--Fourier_add_noise", "Add Gaussian noise in Fourier space");
	stddev_white_noise = textToFloat(parser.getOption("--white_noise", "Standard deviation of added white Gaussian noise", "0"));
	do_add_ice_background = parser.checkOption("--add_ice_background", "Add ice background from a file to output particles");
	fn_Ice_Background = parser.getOption("--ice_background_file", "The ice background mrc file to be added. Should be larger than particle boxsize","");
	Ice_Intensity = textToFloat(parser.getOption("--ice_intensity", "intensity of ice", "1"));
...
void Preprocessing::initialise()
{
	...
	if (do_extract)
	{
		...
		if ((do_phase_flip||do_premultiply_ctf || do_add_ice_background) && !MDmics.containsLabel(EMDL_CTF_DEFOCUSU))
		...
		if (do_phase_flip || do_premultiply_ctf ||do_add_ice_background)
		{
		...
		RFLOAT &all_avg, RFLOAT &all_stddev, RFLOAT &all_minval, RFLOAT &all_maxval)
{
	...
	FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD)
	{
		...
		if (do_add_ice_background)
		{
			Iice.read(fn_Ice_Background);
			long int Iice_Size=YSIZE(Iice());
			if (extract_size > Iice_Size)
				REPORT_ERROR("Your ice background file has size of "+ integerToString(Iice_Size)
			+", which is smaller size than particle extract size of "+ integerToString(extract_size));
			if (extract_size != Iice_Size)
			{
				Iice().window(Iice_Size/2+FIRST_XMIPP_INDEX(my_extract_size), Iice_Size/2+FIRST_XMIPP_INDEX(my_extract_size),
				              Iice_Size/2+ LAST_XMIPP_INDEX(my_extract_size), Iice_Size/2+ LAST_XMIPP_INDEX(my_extract_size));

			}
			FourierTransformer trans_ice;
			MultidimArray<Complex> FT_trans_ice;
			trans_ice.FourierTransform(Iice(), FT_trans_ice, false);
			MultidimArray<RFLOAT> Fctf_ice;
			Fctf_ice.resize(YSIZE(FT_trans_ice), XSIZE(FT_trans_ice));
			ctf.getFftwImage(Fctf_ice, my_extract_size, my_extract_size, my_angpix, false, do_phase_flip, do_ctf_intact_first_peak, true, false);
			FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY(FT_trans_ice)
			{
				DIRECT_MULTIDIM_ELEM(FT_trans_ice, n) *= DIRECT_MULTIDIM_ELEM(Fctf_ice, n);
			}
			transformer.inverseFourierTransform(FT_trans_ice, Iice());

			FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY2D(Iice())
			{
				DIRECT_A2D_ELEM(Ipart(), i, j) += Ice_Intensity*DIRECT_A2D_ELEM(Iice(), i, j);
			}
		}
		...
		RFLOAT &all_maxval)
{
	...
	if(do_addnoise){
		if(fourier_add_noise){
			ADD_NOISE.setReal(Ipart());
			ADD_NOISE.FourierTransform();
			MultidimArray <Complex> &F_ptr_ADD_NOISE=ADD_NOISE.getFourierReference();
			FOR_ALL_ELEMENTS_IN_FFTW_TRANSFORM(F_ptr_ADD_NOISE)
			{
				DIRECT_A2D_ELEM(F_ptr_ADD_NOISE, i, j).real += rnd_gaus(0., stddev_white_noise);
				DIRECT_A2D_ELEM(F_ptr_ADD_NOISE, i, j).imag += rnd_gaus(0., stddev_white_noise);
			}
			ADD_NOISE.inverseFourierTransform();
			Ipart()=ADD_NOISE.getReal();
		}
	//	std::cout<<"debug XSIZE(Ipart())="<<XSIZE(Ipart())<<" YSIZE(Ipart())="<<YSIZE(Ipart())<<"ZSIZE(Ipart())="<<ZSIZE(Ipart())<<std::endl;
		else{
			FOR_ALL_ELEMENTS_IN_ARRAY2D(Ipart()){
				A2D_ELEM(Ipart(), i, j) += rnd_gaus(0., stddev_white_noise);
			}
		}
		if (do_normalise)
		{
