		...
		std::string &final_command, bool do_makedir, int job_counter, std::string &error_message)
{
	...
	{
		result = getCommandsBBRJob(outputname, commands, final_command, do_makedir, job_counter, error_message);
	}
	////
	else if (type == PROC_EXTERNAL)
	{
...
////BBR
void RelionJob::initialiseBBRJob()
{
	hidden_name = ".gui_bbr";
...
//	joboptions["fn_exe"] = JobOption("External executable:", "", "", ".", "Location of the script that will launch the external program. This script should write all its output in the directory specified with --o. Also, it should write in that same directory a file called RELION_JOB_EXIT_SUCCESS upon successful exit, and RELION_JOB_EXIT_FAILURE upon failure.");
	...
	joboptions["in_part"] = JobOption("Input particles:", "", "STAR Files (*.star)", ".", "Particles to perform Block-based Reconstruction.");

	joboptions["in_sym"] = JobOption("Symmetry of particles:", std::string("C1"), "This will be passed with a --sym argument to relion_star_handler.");
	joboptions["in_BBR_X"] = JobOption("X-Position of the reconstruction: ", std::string(""), "X/Y/Z-Position of the reconstruction in pixel. This will be passed with a --BBR_X/Y/Z argument to relion_star_handler.");
	joboptions["in_BBR_Y"] = JobOption("Y-Position of the reconstruction: ", std::string(""), "X/Y/Z-Position of the reconstruction in pixel. This will be passed with a --BBR_X/Y/Z argument to relion_star_handler.");
	joboptions["in_BBR_Z"] = JobOption("Z-Position of the reconstruction: ", std::string(""), "X/Y/Z-Position of the reconstruction in pixel. This will be passed with a --BBR_X/Y/Z argument to relion_star_handler.");
	joboptions["Corr_hand"] = JobOption("Correct handerness", false, "If set to true, this will be passed with a --correct_handerness argument to relion_star_handler. If both set to be false, no defocus change will be made.");
	joboptions["Invert_Corr_hand"] = JobOption("Inverted handerness", false, "If set to true, this will be passed with a --handerness_negative argument to relion_star_handler. If both to be false, no defocus change will be made.");
	joboptions["in_MG_X"] = JobOption("X-size of the micrograph: ", std::string(""), "X/Y-size of the micrograph in pixel. This will be passed with a --micrograph_x/ysize argument to relion_star_handler. If the two arguements >0, it will drop sub-particles lay out side of the micrograph");
	joboptions["in_MG_Y"] = JobOption("Y-size of the micrograph: ", std::string(""), "X/Y-size of the micrograph in pixel. This will be passed with a --micrograph_x/ysize argument to relion_star_handler. If the two arguements >0, it will drop sub-particles lay out side of the micrograph");
	joboptions["IN_ROUND_SHIFT"] = JobOption("Rounded translation", false, "If set to true, this will be passed with a --round_shift argument to relion_star_handler.");
}
bool RelionJob::getCommandsBBRJob(std::string &outputname, std::vector<std::string> &commands,
		std::string &final_command, bool do_makedir, int job_counter, std::string &error_message)
{
	commands.clear();
	initialisePipeline(outputname, PROC_BBR_NAME, job_counter);
	std::string command;
...
*/
	FileName fn_out=outputname+"particles_BBR.star";
	command="relion_star_handler --do_BBR";
	command += " --o " + fn_out;

	// Optional input nodes
	if (joboptions["in_part"].getString() != "")
	{
		command += " --i " + joboptions["in_part"].getString();
	}
	if (joboptions["in_sym"].getString() != "")
	{
		command += " --sym " + joboptions["in_sym"].getString();
	}
	if (joboptions["in_BBR_X"].getString() != "")
	{
		command += " --BBR_X " + joboptions["in_BBR_X"].getString();
	}
	if (joboptions["in_BBR_Y"].getString() != "")
	{
		command += " --BBR_Y " + joboptions["in_BBR_Y"].getString();
	}
	if (joboptions["in_BBR_Z"].getString() != "")
	{
		command += " --BBR_Z " + joboptions["in_BBR_Z"].getString();
	}
	if (joboptions["in_MG_X"].getString() != "")
	{
		command += " --micrograph_xsize " + joboptions["in_MG_X"].getString();
	}
	if (joboptions["in_MG_Y"].getString() != "")
	{
		command += " --micrograph_ysize " + joboptions["in_MG_Y"].getString();
	}
	if (joboptions["Corr_hand"].getBoolean())
	{
		command += " --correct_handerness ";
	}
	if (joboptions["Invert_Corr_hand"].getBoolean())
	{
		command += " --handerness_negative ";
	}
	if (joboptions["IN_ROUND_SHIFT"].getBoolean())
	{
		command += " --round_shift ";
	}
	commands.push_back(command);
