...
////BBR
void JobWindow::initialiseBBRWindow()
{
	setupTabs(1);
	...
	place("in_sym", TOGGLE_DEACTIVATE);
	current_y += STEPY /2 ;
	place3("in_BBR_X", "in_BBR_Y", "in_BBR_Z", "X/Y/Z position of reconstruction", TOGGLE_DEACTIVATE);
//	place("in_BBR_X", TOGGLE_DEACTIVATE);
//	place("in_BBR_Y", TOGGLE_DEACTIVATE);
//	place("in_BBR_Z", TOGGLE_DEACTIVATE);
	current_y += STEPY /2 ;
	place("Corr_hand", TOGGLE_DEACTIVATE);
	place("Invert_Corr_hand", TOGGLE_DEACTIVATE);
	current_y += STEPY /2 ;
	place2("in_MG_X", "in_MG_Y", "X/Y-size of the micrograph", TOGGLE_LEAVE_ACTIVE);
//	place("in_MG_X", TOGGLE_DEACTIVATE);
//	place("in_MG_Y", TOGGLE_DEACTIVATE);
	current_y += STEPY /2 ;
	place("IN_ROUND_SHIFT", TOGGLE_DEACTIVATE);
	tab1->end();
}
////
